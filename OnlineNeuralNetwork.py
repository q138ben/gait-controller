...
class ONN(nn.Module):
    def __init__(self, features_size, max_num_hidden_layers, qtd_neuron_per_hidden_layer, n_classes, loss_fun, batch_size=1,
                 b=0.99, n=0.001, s=0.2, use_cuda=False):
...
#            print ('out = ', out)
            ...
            if self.loss_fun == 'mse':
                ...
                loss = criterion(out.view(self.batch_size, self.n_classes), Y.view(
                    self.batch_size,self.n_classes).float())
    ...
    def predict_(self, X_data):
        ...
        if self.loss_fun == 'mse':
            return torch.sum(torch.mul(
                self.alpha.view(self.max_num_hidden_layers, 1).repeat(1, len(X_data)).view(
                    self.max_num_hidden_layers, len(X_data), 1), self.forward(X_data)), 0).cpu().detach().numpy()
